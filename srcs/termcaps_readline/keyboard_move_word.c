...
/*   Updated: 2016/10/20 16:33:41 by gdeguign         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void move_word_left(t_info *info)
{
  ...
  term = info->term;
	...
	move_cursor(term->capa, term->pos_c , ft_strlen(term->prompt), (pos ? ++pos: 0));
	term->pos_c = pos;
}
...
void move_word_right(t_info *info)
{
  ...
  term = info->term;
	...
	move_cursor(info->term->capa, info->term->pos_c , ft_strlen(term->prompt), pos);
	term->pos_c = pos;
}
